...
| `.agent-handoff/snapshot.md` | 多文档布局下保存当前目标、状态、下一步、活跃文件、阻塞点和开放问题。 |
...
| `AGENTS.md` | Codex 项目级 instructions 文件,只写入一条带标记的 handoff 引用。 |
| `.claude/CLAUDE.md` | 项目级 Claude Code 规则,只写入一条带标记的 handoff 引用。 |
...
它会检查仓库结构,创建 `.agent-handoff/README.md`,并按平台把持久 handoff 引用合并到:
...
如果没有接力机制,新 Agent 只能靠旧聊天恢复上下文。`.agent-handoff/README.md` 会明确记录:
...
```text
使用 agent-handoff skill,审查并修复当前项目的 .agent-handoff/README.md 和相关状态文件。
```
...
| `--gitignore` | 把本地 handoff 文件和 `AGENT_SESSION_PROMPTS.md` 加入 `.gitignore`。 |
...
4. 幂等合并 Codex `AGENTS.md` 的 handoff 引用。
5. 幂等合并 Claude Code `.claude/CLAUDE.md` 的 handoff 引用。
...
```text
使用 agent-handoff skill,修复 .agent-handoff/README.md 和相关状态文件。它们现在太长,而且状态有点乱。
```

Agent 应该做:

1. 读取 `references/quality.md`。
2. 读取当前 `.agent-handoff/README.md` 和相关状态文件。
3. 检查与仓库事实冲突的内容。
4. 压缩陈旧历史。
5. 刷新快照、工作日志、验证历史和待办。
6. 保留有证据的决策,删除聊天流水账。
...
- `.agent-handoff/README.md` 只是入口索引和读取路线,不堆任务日志。
...
- 新 Agent 只读 .agent-handoff/README.md、snapshot、risks、backlog、必要 validation/decisions,就能恢复前一个 Agent 的工作状态。

## 注意事项

- 如果项目决定把 handoff 文件提交进 Git,应谨慎记录内容,避免私密上下文、路径、日志或内部信息泄露。
