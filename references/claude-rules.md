...
将带标记的代码块合并到 Claude Code 的项目级 `.claude/CLAUDE.md`。如果仓库已有 `.claude/CLAUDE.md`，保留现有指引；存在标记块时只替换该标记块。除非用户明确要求，不要编辑用户级 `~/.claude/CLAUDE.md`。
...
对于 `--layout multi`，生成的 Claude Code 规则会指示未来 agent 读取：
...
对于 `--layout single`，生成的 Claude Code 规则会指示未来 agent 读取：
