...
1. 初始化：检查仓库结构，创建或更新 handoff 文件，将项目级 handoff 规则合并到 Codex `AGENTS.md` 和/或 Claude Code `.claude/CLAUDE.md`，可选创建 `AGENT_SESSION_PROMPTS.md`。
2. 会话启动，单文档布局：明确读取 `AGENT_HANDOFF.md`，判断当前活跃子项目，然后只检查与任务相关的源文件。
3. 会话启动，多文档布局：读取 `AGENT_HANDOFF.md`、`.agent-handoff/snapshot.md`、`.agent-handoff/risks.md` 和 `.agent-handoff/backlog.md`；仅在需要时读取其他 handoff 文件。
...
- `AGENT_HANDOFF.md` 只是索引、恢复路径和指针文件。
